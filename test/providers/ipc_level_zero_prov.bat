...
echo "Starting test_ipc_level_zero_prov CONSUMER on port %PORT% ..."
start "" cmd /c ".\%BUILD_TYPE%\test_ipc_level_zero_prov_consumer %PORT% > consumer_log.txt 2>&1"

echo "Waiting 5 sec ..."
ping -n 5 -w 1000 localhost > nul

echo "Starting test_ipc_level_zero_prov PRODUCER on port %PORT% ..."
start "" cmd /c ".\%BUILD_TYPE%\test_ipc_level_zero_prov_producer %PORT% > producer_log.txt 2>&1"
