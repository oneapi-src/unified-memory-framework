...
function print_usage() {
	...
	echo "                 If it is empty, all tests (./test/umf-*) and examples (./examples/umf_example_*)"
...
if [ $(ls -1 ${BUILD_DIR}/test/umf-* 2>/dev/null | wc -l) -eq 0 ]; then
	echo -e "error: UMF tests ./test/umf-* not found in the build directory: ${BUILD_DIR}\n"
	print_usage
	exit 1
fi
...
for test in $TESTS; do
	...
	./test/umf-provider_os_memory_multiple_numa_nodes)
		...
		;;
	./test/umf-memspace_highest_bandwidth)
		FILTER='--gtest_filter="-*allocLocalMt*"'
		;;
	./test/umf-memspace_lowest_latency)
		FILTER='--gtest_filter="-*allocLocalMt*"'
		;;
	./test/umf-memoryPool)
