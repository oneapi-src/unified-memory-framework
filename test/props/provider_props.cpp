...
TEST_F(test, CustomPropsTest) {
    ...
    // get value of the custom property from the provider
    uint64_t value = 0;
    res = umfMemoryProviderGetAllocationProperties(
        provider, ptr, (umf_memory_property_id_t)custom_property_id, &value,
        sizeof(value));
    ASSERT_EQ(res, UMF_RESULT_SUCCESS);
    ASSERT_EQ(value, 42); // Check if the custom property was set correctly
