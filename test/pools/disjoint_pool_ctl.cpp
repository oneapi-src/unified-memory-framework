...
class PoolWrapper {
  ...
  private:
    ...
    umf_memory_pool_ops_t *m_poolOps;
...
class ProviderWrapper {
  public:
    ProviderWrapper(umf_memory_provider_ops_t *providerOps,
                    void *params = nullptr)
        ...
        if (res != UMF_RESULT_SUCCESS) {
            m_provider = nullptr;
            LOG_ERR("Failed to create memory provider");
        }
    }
  ...
  private:
    umf_memory_provider_handle_t m_provider;
    umf_memory_provider_ops_t *m_providerOps;
    void *m_params;
};
...
TEST_F(test, disjointCtlName) {
    ...
    RETURN_SUCCESS(umfCtlSet("umf.pool.default.disjoint.name", NULL,
                             (void *)val, strlen(val)));

    umf_disjoint_pool_params_handle_t params = nullptr;
    RETURN_SUCCESS(umfDisjointPoolParamsCreate(&params));
    ...
    // Clean up
    RETURN_SUCCESS(umfDisjointPoolParamsDestroy(params));
    RETURN_SUCCESS(umfOsMemoryProviderParamsDestroy(os_memory_provider_params));
}
