...
TEST_F(test, disjointPoolTrim) {
    ...
    EXPECT_EQ(pool->buckets[0]->available_slabs_num, (size_t)4);
    EXPECT_EQ(pool->buckets[0]->curr_slabs_in_use, (size_t)0);
    EXPECT_EQ(pool->buckets[0]->curr_slabs_in_pool, (size_t)4);

    // Trim memory - leave 3 slabs in the pool
    ops->ext_trim_memory(pool, 3 * pool->buckets[0]->size);
    EXPECT_EQ(pool->buckets[0]->available_slabs_num, (size_t)3);
    EXPECT_EQ(pool->buckets[0]->curr_slabs_in_use, (size_t)0);

    // Trim memory again - leave 1 slab in the pool
    ops->ext_trim_memory(pool, pool->buckets[0]->size);
    EXPECT_EQ(pool->buckets[0]->available_slabs_num, (size_t)1);
    EXPECT_EQ(pool->buckets[0]->curr_slabs_in_use, (size_t)0);

    // Trim the rest of memory
    ops->ext_trim_memory(pool, 0);
    EXPECT_EQ(pool->buckets[0]->available_slabs_num, (size_t)0);
    EXPECT_EQ(pool->buckets[0]->curr_slabs_in_pool, (size_t)0);
