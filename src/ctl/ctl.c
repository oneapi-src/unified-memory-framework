...
static const struct ctl_node *ctl_find_node(const struct ctl_node *nodes,
    ...
    while (node_name != NULL) {
        char *endptr;
        /*
		 * Ignore ERANGE or EINVA errno from strtol.
		 * endptr is used to check conversion result.
		 */
        int tmp_errno = errno;
...
int ctl_query(struct ctl *ctl, void *ctx, enum ctl_query_source source,
    ...
    const struct ctl_node *n =
        ctl_find_node(CTL_NODE(global, ), name, &indexes);
    ...
    if (n == NULL || n->type != CTL_NODE_LEAF || n->cb[type] == NULL) {
        printf("invalid query entry point %s\n", name);
        errno = EINVAL;
        goto out;
    }
