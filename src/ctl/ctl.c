    ...
    while (node_name != NULL) {
        ...
        if (n->arg != NULL && n->type == CTL_NODE_NAMED) {
            ...
            if (strcmp(next_node, CTL_WILDCARD) == 0) {
                ...
                arg = umf_ba_global_alloc(n->arg->dest_size);
                if (arg == NULL) {
                    goto error;
                }
                pop_va_list(args, n->arg, arg);
            } else {
                arg = ctl_parse_args(n->arg, next_node);
            ...
            if (next_node == NULL) {
                ...
                for (n = n->children; n->type != CTL_NODE_UNKNOWN; ++n) {
                    if (n->type == CTL_NODE_LEAF && *n->name == '\0') {
                ...
                for (const umf_ctl_node_t *m = n->children;
                    ...
                    if (m->type == CTL_NODE_SUBTREE && *m->name == '\0') {
...
static umf_result_t ctl_load_config(struct ctl *ctl, void *ctx, char *buf) {
    ...
    va_list empty_args; // ctl_query need va_list, but we don't have it for
        // config input - ctl_query knows that if query is CTL_QUERY_CONFIG_INPUT, it should not use va_list
