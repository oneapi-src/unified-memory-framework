    ...
    while (node_name != NULL) {
        *name_offset = node_name - parse_str;
        if (n != NULL && n->type == CTL_NODE_SUBTREE) {
            // if subtree occur there should be called subtree handler
            break;
        }
        char *endptr;
        /*
...
static int (*ctl_exec_query[MAX_CTL_QUERY_TYPE])(
    void *ctx, const umf_ctl_node_t *n, umf_ctl_query_source_t source,
    void *arg, umf_ctl_index_utlist_t *indexes, const char *extra_name,
    umf_ctl_query_type_t query_type) = {
    ctl_exec_query_read,
...
int ctl_query(struct ctl *ctl, void *ctx, umf_ctl_query_source_t source,
    ...
    const char *extra_name = (const char *)&name[0] + name_offset;
    ret =
