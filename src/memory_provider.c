...
static umf_result_t umfDefaultPutIPCHandle(void *provider, void *ipcData) {
    (void)provider;
    (void)ipcData;
    return UMF_RESULT_ERROR_NOT_SUPPORTED;
}

static umf_result_t umfDefaultOpenIPCHandle(void *provider, void *ipcData,
                                            void **ptr) {
    (void)provider;
    (void)ipcData;
    (void)ptr;
...
umfMemoryProviderPutIPCHandle(umf_memory_provider_handle_t hProvider,
                              void *ipcData) {
    return hProvider->ops.ipc.put_ipc_handle(hProvider->provider_priv, ipcData);
}

umf_result_t
umfMemoryProviderOpenIPCHandle(umf_memory_provider_handle_t hProvider,
                               void *ipcData, void **ptr) {
    return hProvider->ops.ipc.open_ipc_handle(hProvider->provider_priv, ipcData,
                                              ptr);
}
