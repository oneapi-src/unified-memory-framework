...
typedef struct umf_tracking_memory_provider_t {
    ...
    umf_memory_pool_handle_t pool;
    critnib *ipcCache;
    ipc_handle_mapped_cache_handle_t hIpcMappedCache;
...
static umf_result_t trackingOpenIpcHandle(void *provider, void *providerIpcData,
    ...
    if (mapped_ptr == NULL) {
        ...
        if (mapped_ptr == NULL) {
            ...
            ret = umfMemoryTrackerAdd(p->hTracker, p->pool, mapped_ptr,
                                      bufferSize);
            if (ret != UMF_RESULT_SUCCESS) {
    ...
    *ptr = cache_entry->mapped_base_ptr;
...
static umf_result_t trackingCloseIpcHandle(void *provider, void *ptr,
    ...
    (void)ptr;
    (void)size;
    // Do nothing and keep entry in the p->hIpcMappedCache. The handle is closed when entry is evicted from the p->hIpcMappedCache.
    return UMF_RESULT_SUCCESS;
#endif
}
