...
umf_result_t umfMemoryTrackerGetAllocInfo(const void *ptr,
    ...
    tracker_alloc_info_t *match = NULL;
...
static void check_if_tracker_is_empty(umf_memory_tracker_handle_t hTracker,
    ...
    tracker_address_space_t *address_space = NULL;
    utils_atomic_load_acquire_ptr((void **)&hTracker->address_spaces,
                                  (void **)&address_space);
    for (; address_space; address_space = address_space->next) {
...
umf_result_t umfTrackingMemoryProviderGetMemoryProperties(
    umf_memory_provider_handle_t provider, const void *ptr,
    umf_memory_properties_handle_t *props) {
    if (provider->ops.alloc != trackingAlloc) {
        return umfGetMemoryPropertiesHandle(ptr, props);
    }

    umf_tracking_memory_provider_t *tracking =
        umfMemoryProviderGetPriv(provider);
    tracker_alloc_info_t *info =
        get_alloc_info_from_address_space(tracking->address_space, ptr);
    if (!info) {
        *props = NULL;
        return UMF_RESULT_ERROR_INVALID_ARGUMENT;
    }
