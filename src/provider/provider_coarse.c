...
static block_t *ravl_tree_add_new(struct ravl *rtree, BYTE *data, size_t size) {
    ...
    block_t *block = (block_t *)umf_ba_global_alloc(sizeof(block_t));
    if (block == NULL) {
        return NULL;
    }
...
node_list_add(ravl_free_blocks_head_t *head_node, struct block_t *block) {
    ...
    ravl_free_blocks_elem_t *node =
        (ravl_free_blocks_elem_t *)umf_ba_global_alloc(
            sizeof(ravl_free_blocks_elem_t));
    if (node == NULL) {
        return NULL;
    }
...
static int free_blocks_add(struct ravl *free_blocks, block_t *block) {
    ...
    if (!head_node) {
        head_node = umf_ba_global_alloc(sizeof(ravl_free_blocks_head_t));
        if (!head_node) {
            return -1;
        }

        head_node->head = NULL;
        if (util_mutex_init(&head_node->mutex) == NULL) {
...
static block_t *free_block_merge_with_prev(
    ...
    umf_result_t merge_success = UMF_RESULT_ERROR_UNKNOWN;
    if (upstream_memory_provider && block->prev && block->prev->used == false &&
...
static block_t *free_block_merge_with_next(
    ...
    // check if blocks could be merged by the upstream provider
    umf_result_t merge_success = UMF_RESULT_ERROR_UNKNOWN;
    if (upstream_memory_provider && block->next && block->next->used == false &&
...
static bool debug_check(coarse_memory_provider_t *provider) {
    ...
    while (block) {
        ...
        if (provider->upstream_memory_provider && block->prev &&
            block->prev->used == false && block->used == false &&
            (block->prev->data + block->prev->size == block->data)) {

            umf_result_t merge_success = umfMemoryProviderAllocationMerge(
        ...
        if (provider->upstream_memory_provider && block->next &&
            block->next->used == false && block->used == false &&
            (block->data + block->size == block->next->data)) {
...
static umf_result_t coarse_memory_provider_initialize(void *params,
                                                      void **provider) {
    umf_result_t ret = UMF_RESULT_ERROR_MEMORY_PROVIDER_SPECIFIC;
    ...
    // check params
    // we should either provider an upstream provider or init buffer
    ...
    coarse_memory_provider_t *coarse_provider =
        (coarse_memory_provider_t *)umf_ba_global_alloc(
            sizeof(coarse_memory_provider_t));

    if (!coarse_provider) {
        return UMF_RESULT_ERROR_OUT_OF_HOST_MEMORY;
    }

    if (util_mutex_init(&coarse_provider->lock) == NULL) {
        goto err_free_coarse_provider;
    }

    coarse_provider->trace = coarse_params->trace;
    coarse_provider->WA_do_not_free_upstream =
    ...
    if (coarse_params->immediate_init) {
        // allocate and immediately deallocate memory using the upstream
        // provider
    ...
    } else if (coarse_provider->init_buffer) {
        block_t *alloc = ravl_tree_add_new(coarse_provider->upstream_alloc,
        ...
        coarse_provider->used_size += coarse_params->init_buffer_size;

        if (coarse_provider->trace) {
            printf("coarse_ALLOC (init) %zu used %zu alloc %zu\n",
...
block_error:
    ravl_tree_rm(coarse_provider->upstream_alloc, coarse_provider->init_buffer);
...
static void ravl_cb_rm_upstream_alloc_node(void *data, void *arg) {
    ...
    if (coarse_provider->upstream_memory_provider &&
        coarse_provider->WA_do_not_free_upstream == false) {
        umf_result_t ret =
...
static umf_result_t coarse_memory_provider_alloc(void *provider, size_t size,
    ...
    if (curr && curr->size > size) {
        ...
        // In case of non-zero alignment create an aligned block what would be
        // further used.
        // TODO: we leave some free mem in current block - merge it with prev?
        uintptr_t original = (uintptr_t)curr->data;
        uintptr_t aligned = (original + alignment - 1) & ~(alignment - 1);
        size_t padding = aligned - original;
        if (alignment > 0 && padding > 0) {
            size_t curr_offset = curr->data - curr->origin->data;
            block_t *origin =
                alloc_find_origin(curr->origin, curr_offset + padding);
            assert(origin);

            block_t *aligned_block =
                ravl_tree_add_new(coarse_provider->all_blocks,
                                  curr->data + padding, curr->size - padding);
            ...
            // Calculate new size and use aligned block.
            size -= padding;
            curr = aligned_block;
            assert(curr->size >= size);
        }

        // Split the rest of block and put the new block after the one that we
        // use.
    ...
    if (coarse_provider->upstream_memory_provider == 0) {
        // TODO: OOM
        return UMF_RESULT_ERROR_MEMORY_PROVIDER_SPECIFIC;
    }
...
alloc_error:
    ...
    if (coarse_provider->WA_do_not_free_upstream == false) {
        umfMemoryProviderFree(coarse_provider->upstream_memory_provider,
                              *resultPtr, size);
    }
    return ret;
...
coarse_memory_provider_get_recommended_page_size(void *provider, size_t size,
    ...
    umf_result_t ret = umfMemoryProviderGetRecommendedPageSize(
