...
static umf_result_t devdax_get_ipc_handle(void *provider, const void *ptr,
    ...
    strncpy(devdax_ipc_data->path, devdax_provider->path, NAME_MAX - 1);
    devdax_ipc_data->path[NAME_MAX - 1] = '\0';
...
static umf_result_t devdax_put_ipc_handle(void *provider,
    ...
    if (strncmp(devdax_ipc_data->path, devdax_provider->path, NAME_MAX)) {
        return UMF_RESULT_ERROR_INVALID_ARGUMENT;
    }
...
static umf_result_t devdax_open_ipc_handle(void *provider,
    ...
    fd = os_devdax_open(devdax_provider->path);
    if (fd <= 0) {
